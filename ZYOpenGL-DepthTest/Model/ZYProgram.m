...
+ (GLuint)genTextureWithImageName:(NSString *)imageName {
    ...
    return [self genTextureWithImageName:imageName wrapType:GL_CLAMP_TO_EDGE];
    
}

+ (GLuint)genTextureWithImageName:(NSString *)imageName wrapType:(GLint)wrapType {
...
@implementation ZYVec3

+ (instancetype)vec3WithX:(float)x y:(float)y z:(float)z {
    ZYVec3 *vec3 = [[self alloc] init];
    vec3.x = x;
    vec3.y = y;
    vec3.z = z;
    return vec3;
